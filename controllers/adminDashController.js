...
const getADminDashData = async (req, res) => {
  try {
    ...
    for (const [key, value] of Object.entries(productMap)) {
      ...
      const soldProductCount = await value.countDocuments({ status: "Sold", isPaid: "Not Paid" });
    ...
    const totalProfit = totalSold * 0.5;
    res.status(200).json({
      totalBuyers,
      totalSellers,
      totalAdmins,
  ...
  try {
    ...
    if (filter === 'weekly') {
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        const found = profits.find(p => p._id.week === w.isoWeek() && p._id.year === w.isoWeekYear());
    ...
    console.error(error);
    res.status(500).json({ message: "Something went wrong fetching profit data" });
  }
};

module.exports = {
  getADminDashData, getDashStats, getSalesData, getProfitData
};
